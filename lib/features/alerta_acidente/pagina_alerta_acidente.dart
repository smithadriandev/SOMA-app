...
class _PaginaAlertaAcidenteState extends State<PaginaAlertaAcidente> {
  ...
  final List<_MotivoAlerta> _alertReasons = const [
    _MotivoAlerta(
  ...
  List<Conexao> get _contacts => MockConexoes.conexoes;

  Future<void> _chooseAlertReason() async {
    final reason = await showModalBottomSheet<_MotivoAlerta>(
      context: context,
      backgroundColor: Theme.of(context).cardColor,
      shape: const RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(18)),
      ),
      builder: (context) => _MotivoAlertaSheet(reasons: _alertReasons),
    );
  ...
  void _sendAlert(_MotivoAlerta reason) {
    setState(() {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ..._contacts.map(
                    (contact) => _ItemContatoEmergencia(contact: contact),
                  ),
                  const Spacer(),
                  Center(
...
class _MotivoAlertaSheet extends StatelessWidget {
  const _MotivoAlertaSheet({required this.reasons});

  final List<_MotivoAlerta> reasons;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ...reasons.map(
              (reason) => _MotivoAlertaTile(reason: reason),
            ),
          ],
        ),
      ),
...
class _MotivoAlertaTile extends StatelessWidget {
  const _MotivoAlertaTile({required this.reason});

  final _MotivoAlerta reason;
...
class _ItemContatoEmergencia extends StatelessWidget {
  const _ItemContatoEmergencia({required this.contact});

  final Conexao contact;
